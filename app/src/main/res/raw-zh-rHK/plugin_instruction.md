...
目前階段: 閱讀器以 Readium 預設設定顯示書籍, 提供目錄, 記住每本書的閱讀位置, 提供捲動模式, 點按區, 音量鍵翻頁與沉浸模式, 並提供偏好面板設定字號, 字體, 間距, 對齊, 欄數與主題, 並可匯入自己的 TTF / OTF 字型, 支援 CJK 直排與由右至左的書籍, 並以單頁或雙頁顯示固定版式書籍. 書籤, 搜尋, 朗讀, 獨立啟動入口與 `epub` 指令碼 API 在 ROADMAP.md 中排程, 將在後續版本提供.
