...
現在の段階: リーダーは Readium の既定設定で本を表示し, 目次を提供し, 各書籍の読書位置を記憶し, スクロールモード, タップ領域, 音量キー, 没入モードを提供します. ブックマーク, 設定, 検索, 読み上げ, 固定レイアウト, フォントの取り込み, 独立したランチャー入口, `epub` スクリプト API は ROADMAP.md で管理され, 今後のビルドで提供されます.
