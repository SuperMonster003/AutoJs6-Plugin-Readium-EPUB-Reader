...
Source of truth:
  ...
  gradle/explorer-action-compatibility.properties protocol and vendored AAR audit facts

Generated artifacts (25 in total, never edit them by hand):
...
Validated but not generated:
  ...
  app/src/main/res/raw*/plugin_instruction.md    presence and content hygiene
...
def validate_explorer_action_compatibility(root: Path, common: dict[str, Any]) -> None:
    properties_path = root / "gradle" / "explorer-action-compatibility.properties"
    properties = read_properties(properties_path)
    mappings = {
        "protocol_version": "declaredProtocolVersion",
        "required_host_build": "minimumHostVersionCode",
        "audited_host_build": "maximumAuditedHostVersionCode",
        "audited_host_protocol": "maximumAuditedHostProtocolVersion",
    }
    for common_key, property_key in mappings.items():
        expected = properties.get(property_key, "")
        require(bool(expected), f"{property_key} is missing from {properties_path}")
        require(
            str(common.get(common_key, "")) == expected,
            f".readme/common.json {common_key} does not match {property_key}",
        )

    expected_digest = properties.get("explorerActionApiSha256", "").upper()
    require(
        bool(re.fullmatch(r"[0-9A-F]{64}", expected_digest)),
        f"Invalid explorerActionApiSha256 in {properties_path}",
    )
    api_path = root / "libs" / "explorer-action-api.aar"
    actual_digest = hashlib.sha256(api_path.read_bytes()).hexdigest().upper()
    require(
        actual_digest == expected_digest,
        f"Explorer Action API AAR digest mismatch: expected {expected_digest}, actual {actual_digest}",
    )

    matrix_path = root / str(common.get("protocol_compatibility_path", ""))
    matrix = load_text(matrix_path)
    required_rows = [
        f"| Declared plugin protocol | v{properties['declaredProtocolVersion']} |",
        f"| Minimum host version code | {properties['minimumHostVersionCode']} |",
        f"| Maximum audited host version code | {properties['maximumAuditedHostVersionCode']} |",
        f"| Maximum audited host protocol | v{properties['maximumAuditedHostProtocolVersion']} |",
        f"| Vendored API SHA-256 | `{expected_digest}` |",
    ]
    for row in required_rows:
        require(row in matrix, f"Compatibility matrix is missing or stale: {row}")
...
def validate_localized_resources(root: Path, languages: dict[str, dict[str, Any]]) -> None:
    ...
    for directory in ["raw", *ANDROID_INSTRUCTION_DIRECTORIES.values()]:
        instruction = load_text(resources / directory / "plugin_instruction.md")
        require(bool(instruction.strip()), f"Empty plugin_instruction.md in {directory}")
...
def load_languages(root: Path) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    validate_explorer_action_compatibility(root, common)
