...
- 本仓库是 Explorer Action 家族成员 (与 HTML Previewer / Markdown Previewer 同形), 同时承载 Readium Kotlin Toolkit 阅读器; 独立应用形态 (Launcher, `ACTION_VIEW`, 设置页, 发行历史, 手动更新检查; P4 已落地) 与 TTS 前台服务 (P3 已落地) 的规则见第 6, 14 节; `org.autojs.plugin.EPUB` Binder 服务 (P5.2 已落地: 提取能力; 阅读器会话在 P5.3) 与宿主 `epub-api` 契约的规则见第 6, 7, 8 节.
