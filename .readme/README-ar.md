...
- `تلميح` يغلق 1.1.0 خارطة الطريق P9 (التمييزات والملاحظات والتصدير): يميز القارئ النص المحدد ويضيف إليه ملاحظات, ويسرد التمييزات في لوحة ويصدرها بصيغة Markdown; يتيح الإصدار 2 من عقد EPUB للمضيفين الذين يحملونه قراءة التمييزات عبر `book.annotations()` وتلقي أحداث `highlight`, بينما يواصل AutoJs6 6.8.0 (البنية 5282) العمل بالإصدار 1 من العقد
- `ميزة` التمييزات والملاحظات داخل القارئ (خارطة الطريق P9.2): يميز شريط تحديد النص المقطع باللون المختار آخر مرة أو يفتح محرر الملاحظة (تمييز / تسطير, خمسة ألوان, ملاحظة); تظهر التمييزات في الصفحة ويعيد النقر عليها فتح المحرر; تسرد لوحة "التمييزات والملاحظات" بحسب الفصول وبترتيب القراءة مع الانتقال / التعديل / الحذف / مسح الكل; وتعرض صفحة الإعدادات التمييزات المخزنة وتمسحها
- `ميزة` تصدير تمييزات الكتاب وملاحظاته بصيغة Markdown (خارطة الطريق P9.3): يشارك زر التصدير في اللوحة النص عبر لوحة المشاركة في النظام أو يحفظ ملف `.md` في المكان المختار من منتقي المستندات; يسرد المستند العنوان والمؤلفين والفصول بترتيب القراءة مع كل مقطع وملاحظته ووقته
- `ميزة` الإصدار 2 من عقد EPUB للمضيفين (خارطة الطريق P9.4): يسرد `IEpubBook.getAnnotations` تمييزات الكتاب وملاحظاته بترتيب القراءة صفحة صفحة, وترسل جلسة القارئ حدث `highlight` عند إضافة تمييز أو تعديله أو حذفه; تحافظ الإضافة على الإصدار 1 كخط أساس معلن, وتذكر الإصدار 2 في `epubMaxContractVersion`, وتجيب كل كتاب وكل جلسة بالإصدار الذي حمله طلب الفتح من المضيف, ولا يلاحظ مضيفو الإصدار 1 (AutoJs6 6.8.0 build 5282) أي تغيير
- `تبعية` إضافة `androidx.room:room-runtime` 2.8.1 (قاعدة بيانات التمييزات والملاحظات, خارطة الطريق D4 / P9); يعمل `room-compiler` عبر KSP في وقت البناء فقط
- `تبعية` ترقية `epub-api.aar` إلى بناء الإصدار الخاص بالإصدار 2 من عقد EPUB (وحدة المضيف `plugin-api/epub-api`; يسجل التزام المضيف المصدر والملخص في `libs/README.md` و `locks/host-api-aars.lock`)
