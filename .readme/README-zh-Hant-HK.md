...
> 目前階段 (1.0.0 開發構建): 閱讀器以 Readium 預設設定開啟書籍, 提供目錄, 記住每本書的閱讀位置, 提供捲動模式, 點按區, 音量鍵翻頁與沉浸模式, 並提供偏好面板設定字號, 字體, 間距, 對齊, 欄數與主題 (可跟隨宿主夜間模式). 書籤, 全文搜尋, 朗讀, 固定版式, 字型匯入, 獨立啟動入口與 `epub` 指令碼 API 已在 ROADMAP.md 中排程, 目前尚不可用.
...
可以. 從工具列開啟偏好面板即可設定字號, 字體族 (出版商預設, 襯線, 無襯線, 等寬或 Readium 內建的無障礙字體), 行距, 邊距, 間距, 對齊, 欄數與主題 (淺色, 護眼, 深色或跟隨宿主). 匯入自己的字型檔案將隨後續里程碑提供.
