...
> 1.1.0 是目前版本, 1.0.0 是首個正式版本. 閱讀器開啟 EPUB 2 與 EPUB 3 書籍並提供目錄, 記住每本書的閱讀位置, 提供捲動模式, 點按區, 音量鍵翻頁與沉浸模式, 偏好面板 (字號, 字型, 間距, 對齊, 欄數與可跟隨宿主夜間模式的主題), 匯入的 TTF / OTF 字型, CJK 直排與從右到左的書籍, 以單頁或雙頁顯示的固定版式書籍, 全文搜尋, 書籤, 書內連結, 註釋與圖片, 以及使用系統文字轉語音引擎的朗讀. 應用程式圖示開啟帶最近書籍與系統文件選擇器的啟動器, 其它應用程式可透過 `ACTION_VIEW` 交來 EPUB, 設定頁涵蓋閱讀器預設值, 裝置上儲存的資料與手動更新檢查. `epub` 指令碼 API, 宿主閱讀器工作階段與三份範例指令碼隨 AutoJs6 6.8.0 (版本號 5282) 提供. 1.1.0 新增標示與筆記 (ROADMAP.md, P9): 選取的文字可以用四種顏色標示或加底線並附上筆記, 標示繪製在頁面上並在面板中列出 (跳轉, 編輯, 刪除), 一本書的標示與筆記可以經系統分享匯出為 Markdown 或儲存為檔案; 攜帶 EPUB 契約版本 2 的宿主 (比 5282 更新的 AutoJs6 構建) 透過 `book.annotations()` 讀取它們並在閱讀器工作階段上收到 `highlight` 事件, AutoJs6 6.8.0 (版本號 5282) 仍以契約版本 1 正常運作.
...
- `提示` 1.1.0 完成路線圖 P9 (標示, 筆記與匯出): 閱讀器對選取的文字標示並添加筆記, 在面板中列出標示並匯出為 Markdown; EPUB 契約版本 2 讓攜帶它的宿主透過 `book.annotations()` 讀取標示並收到 `highlight` 事件, AutoJs6 6.8.0 (版本號 5282) 仍以契約版本 1 正常運作
