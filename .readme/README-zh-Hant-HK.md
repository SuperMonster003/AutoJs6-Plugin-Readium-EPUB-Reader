...
> 目前階段 (1.0.0 開發構建): 閱讀器以 Readium 預設設定開啟書籍, 提供目錄, 記住每本書的閱讀位置, 並提供捲動模式, 點按區, 音量鍵翻頁與沉浸模式. 書籤, 偏好設定, 全文搜尋, 朗讀, 固定版式, 字型匯入, 獨立啟動入口與 `epub` 指令碼 API 已在 ROADMAP.md 中排程, 目前尚不可用.
...
4. 使用工具列上的目錄按鈕在章節間跳轉, 點按頁面左右三分之一或按音量鍵翻頁, 點按中央隱藏或顯示工具列; 按返回鍵關閉閱讀器, 閱讀位置會被記住.
...
暫時不能. 閱讀偏好 (字型, 字號, 行距, 邊距, 主題) 將隨偏好設定里程碑提供; 目前構建提供捲動或分頁模式, 其餘使用 Readium 預設值.
