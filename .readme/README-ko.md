...
> 현재 단계 (1.0.0 개발 빌드): 리더는 Readium 기본 설정으로 책을 열고, 목차를 제공하며, 각 책의 읽기 위치를 기억하고, 스크롤 모드, 탭 영역, 볼륨 키, 몰입 모드를 제공합니다. 북마크, 환경 설정, 전체 텍스트 검색, 읽어 주기, 고정 레이아웃, 글꼴 가져오기, 독립 실행 진입점과 `epub` 스크립트 API는 ROADMAP.md에 계획되어 있으며 아직 사용할 수 없습니다.
...
4. 툴바의 목차 버튼으로 챕터 사이를 이동하고, 페이지의 왼쪽이나 오른쪽 3분의 1을 탭하거나 볼륨 키를 눌러 페이지를 넘기며, 가운데를 탭해 툴바를 숨기거나 표시합니다; 뒤로 가기를 누르면 리더가 닫히고 위치가 기억됩니다.
...
아직은 아닙니다. 읽기 환경 설정 (글꼴, 크기, 줄 간격, 여백, 테마)은 환경 설정 마일스톤에서 제공됩니다. 현재 빌드는 스크롤 또는 페이지 모드를 제공하며 나머지는 Readium 기본값을 사용합니다.
