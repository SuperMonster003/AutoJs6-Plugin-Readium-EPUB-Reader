...
> 当前阶段 (1.0.0 开发构建): 阅读器以 Readium 默认设置打开书籍并提供目录. 阅读位置记忆, 书签, 偏好设置, 全文搜索, 朗读, 固定版式, 字体导入, 独立启动入口与 `epub` 脚本 API 已在 ROADMAP.md 中排期, 目前尚不可用.
...
阅读位置记忆与书签属于 ROADMAP.md 的下一个里程碑. 当前构建总是从书籍开头打开.

#### 可以更改字体, 字号或主题吗?

暂时不能. 阅读偏好 (字体, 字号, 行距, 边距, 主题, 分页或滚动模式) 将随偏好设置里程碑提供; 当前构建使用 Readium 默认值.
