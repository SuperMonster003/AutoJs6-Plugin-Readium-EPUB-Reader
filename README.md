...
- `提示` 1.1.0 完成路线图 P9 (高亮, 笔记与导出): 阅读器对选中的文本高亮并添加笔记, 在面板中列出高亮并导出为 Markdown; EPUB 契约版本 2 让携带它的宿主通过 `book.annotations()` 读取高亮并收到 `highlight` 事件, AutoJs6 6.8.0 (版本号 5282) 仍以契约版本 1 正常工作
- `新增` 阅读器内的高亮与笔记 (路线图 P9.2): 文本选择工具栏可用上次选择的颜色高亮所选段落, 或打开笔记编辑器 (高亮 / 下划线, 五种颜色, 笔记); 高亮渲染在页面中, 点按即可重新打开编辑器; "高亮与笔记" 面板按章节以阅读顺序列出, 支持跳转 / 编辑 / 删除 / 全部清除; 设置页显示并可清除已存储的高亮
- `新增` 将一本书的高亮与笔记导出为 Markdown (路线图 P9.3): 面板的导出按钮可经系统分享面板分享文本, 或将 `.md` 文件保存到文档选择器中选定的位置; 文档按阅读顺序列出书名, 作者与各章节, 含每段引文, 其笔记与时间
- `新增` 面向宿主的 EPUB 契约版本 2 (路线图 P9.4): `IEpubBook.getAnnotations` 按阅读顺序分页列出一本书的高亮与笔记, 阅读器会话在高亮被添加, 编辑或删除时发送 `highlight` 事件; 插件仍以契约版本 1 作为对外声明的基线, 在 `epubMaxContractVersion` 中声明版本 2, 每本书与每个会话均以宿主打开请求所携带的版本应答, 版本 1 的宿主 (AutoJs6 6.8.0 build 5282) 不受影响
- `依赖` 附加 `androidx.room:room-runtime` 2.8.1 (高亮与笔记数据库, 路线图 D4 / P9); `room-compiler` 仅在构建期经 KSP 运行
- `依赖` 升级 `epub-api.aar` 至 EPUB 契约版本 2 的发布构建 (宿主模块 `plugin-api/epub-api`; 来源宿主提交与摘要记录于 `libs/README.md` 与 `locks/host-api-aars.lock`)
