...
- P1.2 + P1.3 接线 (`1d880ab`): `EpubReaderViewModel` (打开 / 临时键 / 别名解析 / 后台全量哈希迁移 / 节流持久化), `EpubReaderActivity` (chrome, 目录, 点按区, 音量键, 滚动模式, 从头开始, 就绪门控), `ReaderChrome` / `TocSheet` / `PageTurnPolicy` / `ReaderProgress` / `TocFlattener` / `ReaderSettings`, 布局与菜单, 11 目录 21 键字符串, 10 语言 changelog / README / 说明书.
