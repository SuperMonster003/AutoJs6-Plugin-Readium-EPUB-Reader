...
- [ ] (发布) GitHub Release `v1.0.0` (签名 APK + 变更摘要); 维护者确认后推送. 准备就绪 (2026-09-21): 现有 Release `v1.0.0` (2026-09-20, build 42, `c7a3159`) 将按 Angus Mail 先例原地重切到 `ca3044b` (build 65): 标签移动, 资产替换为 `autojs6-plugin-readium-epub-reader-v1.0.0-d84b0ec1.apk` 与 `SHA256SUMS`, 说明改为 changelog 正文 (不含开发构建提示); 待维护者确认后执行.
- [ ] (索引) `AutoJs6-Official-Plugins-Index`: 运行 `tools/generate_official_plugin_index.py` 生成条目, `release-manifests/io.github.supermonster003.autojs6.plugin.readium.epub.reader/<versionCode>.json` 准入清单; 宿主插件中心能看到并安装. 准备就绪 (2026-09-21): 准入清单 `release-manifests/io.github.supermonster003.autojs6.plugin.readium.epub.reader/65.json` 已写入本地索引仓库 (schemaVersion 1, sourceCommit `ca3044b`, 签名 `31a681fc...c213`, 资产名 / SHA-256 / 大小), `tools/generate_official_plugin_index.py` 需在 Release 重切后运行 (它从 GitHub Releases 取资产并按 packageName + versionCode 匹配清单); 提交与推送待维护者确认.
...
验收: `git status --short` 为空, `VERSION_BUILD` 与提交数一致, Release 资产 CRC32 与文件一致, 索引条目可被宿主消费. 验收结论 (2026-09-21): 发布源提交 `ca3044b` 的工作树干净, `VERSION_BUILD` 65 = 提交数; 资产 CRC32 `d84b0ec1` 与文件一致; Release 重切与索引条目 (P8.4 / P8.5) 已准备, 待维护者确认后推送, 之后补记 "索引条目可被宿主消费" 的验证.
...
- P8.4 / P8.5: Release 重切与索引清单 65.json 已准备, 待维护者确认.
...
- 下一步: 维护者确认后重切 Release `v1.0.0`, 运行索引生成器并推送索引; P9 (高亮, 笔记, 导出) 规划; Pad 解锁后补跑界面类; P0.3 外部样本仍待网络许可.
