...
- [ ] (索引) `AutoJs6-Official-Plugins-Index`: 运行 `tools/generate_official_plugin_index.py` 生成条目, `release-manifests/io.github.supermonster003.autojs6.plugin.readium.epub.reader/<versionCode>.json` 准入清单; 宿主插件中心能看到并安装. 证据 (2026-09-21, 维护者确认后执行): 准入清单 `release-manifests/io.github.supermonster003.autojs6.plugin.readium.epub.reader/65.json` (schemaVersion 1, sourceCommit `ca3044b`, 签名 `31a681fc...c213`, 资产名 / SHA-256 / 大小); Release 重切后运行 `tools/generate_official_plugin_index.py` (生成器 42 项单测通过, 60 个官方仓库全部索引), 生成文件的差异只有 Readium 条目: versionCode 42 -> 65, 资产 `autojs6-plugin-readium-epub-reader-v1.0.0-d84b0ec1.apk` (3,340,340 B, SHA-256 `e8c50ebf...892b`, signerSha256 `31a681fc...c213`, sourceCommit `ca3044b`), changelogText 为新说明, nativePageAlignment 0 (measured); 索引提交 `421732d` (`chore(index): admit Readium EPUB Reader 1.0.0 (build 65)`) 推送到 `main`, 索引仓库 Plugin Index 工作流 (运行 35570768863) 成功且未再产生后续提交; 宿主读取的 `raw.githubusercontent.com/SuperMonster003/AutoJs6-Official-Plugins-Index/refs/heads/main/plugins.official.generated.json` 已返回 versionCode 65, 资产 URL HEAD 200 且 Content-Length 3,340,340. 宿主消费验证: Redmi 12C (API 33, 宿主 6.8.0 build 5282) 打开插件中心 (活动未导出, 用宿主内脚本 `build/p85-center.js` 启动) 后在线拉取索引, 宿主缓存 `files/plugin_center/autojs6_plugin_index_v9.json` 内 Readium 条目 versionCode 65, 资产 `...-d84b0ec1.apk`, 卡片显示 "Readium EPUB Reader 1.0.0 (65) | 2026-09-21", 已安装 build 65 故 Update 置灰 (截图 `build/p85-center-redmi.png`).
